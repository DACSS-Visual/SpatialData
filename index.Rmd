...
We can see the polygons like this using _ggplot2_:
...
As maps are a two dimensional representation of a three dimensional object, there is a particular projection depending on the location of the map on the earth. The current projection for WA state is:

```{r}
st_crs(zipMap)
```

When you get the EPSG as 4326, it is better to have the right projection to compute distances, because the current one (which is generallt the default when we download maps) is not useful for that. Then,

```{r}
# you can search or query the web/AI for the right CRS!!!
...
# for this computation we use 'zipMap_proj'
...
zipMap$area_km2=drop_units(set_units(st_area(st_make_valid(zipMap_proj)),'km2'))
